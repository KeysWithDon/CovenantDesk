...
const rentedAreas = ["Sanctuary", "Fellowship hall", "Classroom", "Conference room", "Office", "Kitchen", "Gymnasium", "Auditorium", "Outdoor area", "Entire building"];
...
function DateListField({ form, name, label, help }: { form: Form; name: "term.specialEventDates" | "term.unavailableDates"; label: string; help: string }) {
  const values = form.watch(name);
  return <label className="field"><span>{label}</span><textarea rows={3} value={values.join("\n")} placeholder="YYYY-MM-DD, one date per line" onChange={(event) => form.setValue(name, event.target.value.split(/[\n,]+/).map((item) => item.trim()).filter(Boolean), { shouldDirty: true, shouldValidate: true })} /><small>{help}</small></label>;
}

function RentalPatternChooser({ form }: { form: Form }) {
  const pattern = form.watch("term.rentalPattern");
  const setPattern = (next: ContractData["term"]["rentalPattern"]) => {
    form.setValue("term.rentalPattern", next, { shouldDirty: true, shouldValidate: true });
    if (next === "one-day") {
      form.setValue("term.threeMonthCommitmentEnabled", false, { shouldDirty: true, shouldValidate: true });
      const start = form.getValues("term.startDate");
      if (start) form.setValue("term.endDate", start, { shouldDirty: true, shouldValidate: true });
    }
  };
  const choices: { value: ContractData["term"]["rentalPattern"]; label: string }[] = [
    { value: "one-day", label: "One day" },
    { value: "recurring-weekly", label: "Recurring weekly" },
    { value: "multi-day", label: "Multi-day" },
    { value: "custom", label: "Custom schedule" },
  ];
  return <div className="choice-section"><h4>Rental pattern</h4><div className="choice-grid">{choices.map((choice) => <button type="button" key={choice.value} className={pattern === choice.value ? "selected" : ""} aria-pressed={pattern === choice.value} onClick={() => setPattern(choice.value)}>{pattern === choice.value && <Check size={13} />}{choice.label}</button>)}</div></div>;
}
...
export function ContractForm({ form }: { form: Form }) {
  ...
  const completion = {
    1: Boolean(values.metadata.contractNumber && values.metadata.effectiveDate), 2: Boolean(values.property.address.street && (values.property.rentedAreas.length || values.property.customArea)), 3: Boolean(values.lessor.legalName && values.lessor.responsible.fullName && values.lessor.responsible.hasAuthority), 4: Boolean(values.renter.legalName && values.renter.responsible.fullName && values.renter.responsible.hasAuthority), 5: Boolean(values.term.rentalPattern && values.term.startDate && values.term.endDate && (values.term.rentalPattern !== "one-day" || (!values.term.threeMonthCommitmentEnabled && values.term.startDate === values.term.endDate))), 6: values.schedule.length > 0 && values.schedule.every((entry) => entry.rentalStart && entry.rentalEnd), 7: values.payment.rentalPrice >= 0 && Boolean(values.payment.frequency), 8: values.securityDeposit.amount >= 0 && (values.securityDeposit.amount === 0 || Boolean(values.securityDeposit.dueDate)), 9: Boolean(values.notices.lessorAddress && values.notices.renterAddress), 10: values.propertyRules.length > 0, 11: !values.insurance.required || Boolean(values.insurance.policyLimit), 12: values.defaults.selectedDefaults.length > 0, 13: true, 14: Boolean(values.governingLaw.state && values.governingLaw.county), 15: true, 16: Boolean(values.signatureMethod), 17: values.metadata.attorneyNoticeAccepted,
  ...
  return (
    <div className="contract-form" aria-label="Contract form">
      ...
      <Accordion number={2} title="Property information" subtitle="Premises, rented areas, access, and permitted use" complete={completion[2]} open={openSections.includes(2)} onToggle={() => toggle(2)}>
        ...
        <div className="field-grid two-column"><Field form={form} name="property.entryDoors" label="Approved entry doors" textarea /><Field form={form} name="property.areasIncluded" label="Areas included" textarea /><Field form={form} name="property.areasExcluded" label="Areas excluded" textarea /><Field form={form} name="property.accessInstructions" label="Property access instructions" textarea /><Field form={form} name="property.keyInstructions" label="Key or access-code instructions" textarea /><Field form={form} name="property.permittedUse" label="Permitted use" textarea /><Field form={form} name="property.prohibitedUses" label="Prohibited uses" textarea /><Field form={form} name="property.limitations" label="Special property limitations" textarea /></div>
      ...
      <Accordion number={5} title="Rental term" subtitle="Rental pattern, dates, termination, and renewal" complete={completion[5]} open={openSections.includes(5)} onToggle={() => toggle(5)}>
        <RentalPatternChooser form={form} />
        <div className="field-grid three-column"><Field form={form} name="term.startDate" label="Contract start date" type="date" /><Field form={form} name="term.endDate" label="Contract end date" type="date" help={values.term.rentalPattern === "one-day" ? "Use the same date as the start date." : undefined} /><Field form={form} name="term.initialAccessDate" label="Initial access date" type="date" /><Field form={form} name="term.initialAccessTime" label="Initial access time" type="time" /><SelectField form={form} name="term.termType" label="Term structure" options={["fixed", "month-to-month", "auto-renew", "non-renewing", "custom"]} /><Field form={form} name="term.noticePeriodDays" label="Termination notice (days)" type="number" /><Field form={form} name="term.renewalPeriod" label="Renewal period" /><Field form={form} name="term.renewalDate" label="Renewal date" type="date" /><Field form={form} name="term.renewalNoticeDays" label="Renewal notice (days)" type="number" /><Field form={form} name="term.finalVacateDate" label="Final property-vacate date" type="date" /><Field form={form} name="term.finalKeyReturnDate" label="Final key-return date" type="date" /></div>
        {values.term.rentalPattern !== "one-day" && <Toggle form={form} name="term.threeMonthCommitmentEnabled" label="Use an initial three-month commitment" help="When selected, early voluntary termination may forfeit the deposit; after the period, 15 days’ written notice applies." />}
      ...
      <Accordion number={6} title="Rental days and times" subtitle="Approved schedule, special events, and blocked dates" complete={completion[6]} open={openSections.includes(6)} onToggle={() => toggle(6)}><ScheduleBuilder form={form} /><div className="field-grid two-column"><DateListField form={form} name="term.specialEventDates" label="Special event dates" help="These dates are included even when they differ from the usual schedule." /><DateListField form={form} name="term.unavailableDates" label="Unavailable dates" help="The agreement excludes these dates from a recurring schedule." /></div></Accordion>
      ...
      <Accordion number={7} title="Rental price and payment" subtitle="Pricing method, due dates, fees, and payment summary" complete={completion[7]} open={openSections.includes(7)} onToggle={() => toggle(7)}>
        ...
        <div className="field-grid two-column"><Field form={form} name="payment.payeeName" label="Payee name" /><Field form={form} name="payment.paymentAddress" label="Mailing address for payments" /><Field form={form} name="payment.electronicInstructions" label="Electronic payment instructions" textarea /><Field form={form} name="payment.additionalPaymentNotes" label="Additional payment notes" textarea /></div>
      ...
      <Accordion number={8} title="Security deposit" subtitle="Amount, payment record, deductions, and refund" complete={completion[8]} open={openSections.includes(8)} onToggle={() => toggle(8)} locked>
        <div className="field-grid three-column"><Field form={form} name="securityDeposit.amount" label="Security-deposit amount" type="number" prefix="$" /><Field form={form} name="securityDeposit.dueDate" label="Deposit due date" type="date" help="Not required when the deposit is $0." /><SelectField form={form} name="securityDeposit.status" label="Payment status" options={["Not due", "Due", "Paid", "Partially paid", "Refunded"]} /><Field form={form} name="securityDeposit.paymentMethod" label="Payment method" /><Field form={form} name="securityDeposit.datePaid" label="Date paid" type="date" /><Field form={form} name="securityDeposit.receiptNumber" label="Receipt number" /><Field form={form} name="securityDeposit.receivedBy" label="Person who received deposit" /><Field form={form} name="securityDeposit.recordReference" label="Account or record reference" /><Field form={form} name="securityDeposit.refundMethod" label="Refund payment method" /><Field form={form} name="securityDeposit.inspectionDays" label="Inspection and refund period (days)" type="number" /></div>
      ...
      <Accordion number={12} title="Default, breach, and cancellation" subtitle="Default events, cure, emergencies, and force majeure" complete={completion[12]} open={openSections.includes(12)} onToggle={() => toggle(12)}>
        ...
        {values.term.rentalPattern === "one-day" ? <div className="major-rights-box"><h4>One-day cancellation policy</h4><SelectField form={form} name="cancellation.policyType" label="Cancellation choice" options={["fully-refundable", "partially-refundable", "nonrefundable-reservation", "custom"]} />{values.cancellation.policyType !== "custom" && <Field form={form} name="cancellation.refundUntil" label="Cancellation deadline" type="date" />}{values.cancellation.policyType === "partially-refundable" && <Field form={form} name="cancellation.partialRefundPercent" label="Refund percentage" type="number" />}{values.cancellation.policyType === "nonrefundable-reservation" && <Field form={form} name="cancellation.reservationPayment" label="Nonrefundable reservation payment" type="number" prefix="$" />}{values.cancellation.policyType === "custom" && <Field form={form} name="cancellation.customPolicy" label="Custom cancellation policy" textarea />}</div> : <p className="section-note"><Info size={15} /> Termination follows the commitment selection and notice period in the Rental term section.</p>}
      </Accordion>
      ...
      <Accordion number={16} title="Signature method" subtitle="Wet-ink, electronic, or hybrid execution" complete={completion[16]} open={openSections.includes(16)} onToggle={() => toggle(16)} locked>
        ...
        <Toggle form={form} name="signatureOptions.witnessEnabled" label="Add optional witness signature line" />
        <Toggle form={form} name="signatureOptions.notaryEnabled" label="Add optional notary acknowledgment" />
      </Accordion>
